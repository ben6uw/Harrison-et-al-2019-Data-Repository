---
title: "Ben Harrison global metabolomics data positive mode - reanalyze pid1652"
author: Lu Wang
output:
...
#NormalizeMets::Dendrogram(t(raw.matrix),samps$block)
#NormalizeMets::Dendrogram(t(impute.matrix),samps$block)
#NormalizeMets::RlaPlots(t(raw.matrix),samps$comb,type = "wg")
#NormalizeMets::RlaPlots(t(impute.matrix),samps$comb,type = "wg")
#----------------------------------------------
# check drifting
...
for (i in 1:length(unique(samps$comb))){
  subset = samps[samps$comb ==levels(samps$comb)[i],]
  pca_set3(noBatch[, match(subset$Sample.ID,colnames(noBatch))], subset, 
           title= paste0("noBatch_3",datamode,levels(samps$comb)[i]))
}

noBatch_pca= pca_set3(noBatch, samps, title=paste0("noBatch_",datamode))$pca_df
...
summary(lm(noBatch_pca$PC3 ~samps$runOrder+samps$comb))
summary(lm(noBatch_pca$PC3 ~samps$block+samps$comb))

summary(lm(noBatch_pca$PC3 ~samps$sampleWeight+samps$line.weight+samps$comb))# 
summary(lm(noBatch_pca$PC3 ~samps$sampleWeight+samps$runOrder+samps$comb))# 
#---------------------------------------------------------
# use limma pipeline
#----------------------------------------
levels(samps$comb)
...
Below shows a series of PCA plots that are coded by different variables using the $log_2$ transformed raw data. PC2 separates the trait (e.g. resistant vs. sensitive group). PC2 & PC3 separates the batches (block column). 
...
Below is a series of PCA plots after removing batch as a covariate on the pre-processed data. Note that we will include the batch as a covariate in the model directly. This step is just to check if there is any other confounders that need to be adjusted in the model.

Overall, PC1 may be related to sample missingness. PC2 separates the resistant vs. sensitive groups, and to some extent separates the control and $H_2O_2$ treated flies in the sensitive group but not in the resistant group. PC2 & PC3 also may related to sample weight. 
